...
class ReportDownloadTests(unittest.TestCase):
    ...
    def test_download_center_exposes_word_markdown_and_data_artifacts(self):
        source = (ROOT / "index.html").read_text(encoding="utf-8")
        for element_id in (
            "downloadReportTemplateDocx",
            "downloadReportTemplateMd",
            "downloadReportInput",
        ...
        self.assertLess(
            source.index('id="downloadReportTemplateDocx"'),
            source.index('id="downloadReportTemplateMd"'),
        )
