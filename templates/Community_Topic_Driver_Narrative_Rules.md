...
核心驱动因素不设置固定的正向、中性或负向组合，也不规定情绪标签的先后顺序。同一周的三个核心驱动可以出现相同情绪，具体分布必须由真实讨论证据和话题影响力决定，不得为了情绪平衡而调整排序或抬升代表性不足的话题。
...
3. 情绪分布应真实反映本周讨论，不得为了形成正向、中性、负向的表面均衡而改变话题入选结果或排序；
4. 只有当本周经过聚类、去重和质量检查后，合格的独立话题不足10个时，才允许少于10个；
...
1. 对聚类后的独立话题进行去重；
2. 依据讨论规模、互动热度、业务影响、玩家体验影响、版本或赛事重要性及跨周延续性，对全部合格话题进行综合影响力排序；
3. 将综合影响力最高的3个话题列为核心驱动因素，不限制其情绪标签组合；
4. 从其余合格话题中按综合影响力选出7个补充驱动因素；
5. 默认输出总计10个话题，并保持整体从高影响力到低影响力的顺序；
6. 不得为了让 Positive、Neutral、Negative 数量均衡而替换、抬升或压低话题；
7. 若合格话题不足10个，则仅输出实际合格数量，不得补写、拆分或重复话题。
...
- [ ] 未新增或删除字段；
- [ ] 每个话题均包含标题、情绪标签和正文；
- [ ] 核心话题位于表格前部，并按综合影响力从高到低排列；
- [ ] 默认共输出10个互不重复的话题，其中前3个为核心驱动、后7个为补充驱动；
- [ ] 核心驱动未被强制设置为正向、中性、负向各1个；
- [ ] 未为了情绪标签均衡而改变话题入选结果或影响力排序；
...
- 每周默认输出10个话题，其中综合影响力最高的3个为核心驱动因素，其余7个为补充驱动因素；全部话题按综合影响力排序，不设置固定情绪组合或顺序。仅在合格独立话题不足时允许减少数量，但最低为8个。
- 后续报告必须通过 `config/weekly_bilingual_report_contract.json` 定义的 W30 结构契约与 `scripts/validate_weekly_report_contract.py` 自动校验。
