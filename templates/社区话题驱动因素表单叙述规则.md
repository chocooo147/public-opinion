...
规则版本：`event-driver-four-class-sentiment-2026-08-12`
...
| 判断层 | Positive / Neutral / Mixed / Negative | 标记话题情绪方向 |
...
Neutral仅用于讨论的主要内容属于机制理解、信息讨论、问题定义、分析、解释或验证，且尚未形成明确的价值倾向。
...
正文必须明确该 Neutral 属于原因争议、信息讨论或结论尚未形成。

不得因正向和负向评价同时存在就默认使用 Neutral。

---

### 6.3 Mixed

Mixed仅用于正向和负向反应都具有实质性、可追溯的证据支持，且没有明显主导方向的情况。

判断时检查：

- 两种方向是否都在多个或具有代表性的真实证据中出现；
- 两种方向是否都对理解该话题有实质作用；
- 是否存在覆盖面、重复出现情况、来源范围或代表性明显更强的一方。

若一方明显主导，使用 Positive 或 Negative，并在正文保留另一方的少量或次要意见。不得设置固定百分比或其他未经批准的数值阈值。
...
```text
Players welcomed [positive aspect], but concerns remained around [negative aspect]. Both directions had substantive, traceable support, with neither clearly dominating.
```

中文规则：

> 玩家认可【正向元素】，但对【负向元素】也存在实质性不满；两种反应均有可追溯证据支持，且没有明显主导方向，因此整体反应为 Mixed。
...
先识别是否存在明显主导方向，再依据第6节判断 Positive、Neutral、Mixed 或 Negative。不得将正负冲突默认归为 Neutral；只有两种方向均具实质证据且无明显主导时才使用 Mixed。
...
6. 不得为了让 Positive、Neutral、Mixed、Negative 数量均衡而替换、抬升或压低话题；
...
- [ ] Positive有明确认可依据；
- [ ] Neutral仅用于机制理解、信息讨论、问题定义、分析、解释或验证，未被当作正负冲突的默认兜底；
- [ ] Mixed同时具有正向和负向的实质性、可追溯证据，且没有明显主导方向；
- [ ] Negative说明了具体问题；
- [ ] 主导 Positive 或 Negative 已保留有意义的少量或次要不同意见，不因其存在而改判 Mixed；
- [ ] 未使用固定百分比或未经批准的数值阈值判断 Mixed；
- [ ] 情绪标签与正文一致。
...
- 只写话题名称，不解释原因；
- 只重复 Positive、Neutral、Mixed 或 Negative；
- 将两个无关问题放入同一话题；
- 将少量玩家意见写成全体共识；
...
```text
Players welcomed the collaboration concept and several visual details, but also raised sustained concerns around the Legend’s facial model. Both reactions were supported by recurring evidence, with neither direction clearly dominating.
```

### Neutral

**Topic / Driver:** Ranked System Explanation
**Sentiment:** Neutral

```text
Players discussed how the Ranked system calculates placement and compared different explanations of its mechanics. The conversation focused on understanding the rules rather than reaching a clear positive or negative evaluation.
```
