...
GitHub Pages 入口为 <https://chocooo147.github.io/public-opinion/>。当前页面包含：

- 综合、B站、小黑盒三种视图。综合视图并列展示两个平台，但不把B站评论数与小黑盒可见帖子数合并解释为正式跨平台总量。
...
预设只读演示账号为apex。该账号只能浏览看板，不显示报告下载和 JSON 导入入口。管理员创建普通账号时可选择“可下载与导入”或“只读”，并可后续修改该权限。

账号与权限判断运行在静态页面前端，仅用于演示界面分级，不等同于服务器端身份认证或数据安全边界。不要在该机制下放置任何敏感数据。

后续 Excel 周报固定复用 W30 的单工作表左右中英对照结构。每周目标为 10 个独立驱动因素，只有证据合格的独立驱动不足 10 个时才允许输出 8 或 9 个，禁止拆分或重复话题凑数。结构契约位于 `config/weekly_bilingual_report_contract.json`，交付前运行：
